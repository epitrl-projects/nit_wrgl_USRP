...
def parse_args():
    ...
    parser.add_argument("--tx_rate", type=float,default=50e6, help="specify to perform a TX rate test (sps)")
...
def benchmark_tx_rate(usrp, tx_streamer, random, timer_elapsed_event, tx_statistics):
    ...
    # TODO: 
    # 
    # The C++ code uses rx_cpu type here. Do we want to use that to set dtype?
    # transmit_buffer = np.zeros((num_channels, max_samps_per_packet), dtype=np.complex64)
    # Create a transmit buffer with zeros
    transmit_buffer = np.zeros((num_channels, max_samps_per_packet), dtype=np.complex64)

    # Convert the message bytes to complex numbers
    byte_message = "hello world".encode('utf-8')
    message_length = len(byte_message)

    # Fill the transmit buffer with the encoded message
    for i in range(message_length):
        transmit_buffer[0, i] = byte_message[i] + 0j  # Assign real part as byte value and imaginary part as 0
